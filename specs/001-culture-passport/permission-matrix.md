...
> ใช้เป็นเอกสาร reference สำหรับทีมพัฒนาในการ implement Route Guards, API Authorization, และ UI Visibility
...
| Attach file | File Upload ใน Mission Modal | Missions | Upload deliverable |
...
|-----------|-------|---------|----------|---------|
| **Navbar** | ✅ | ✅ | ✅ | ชื่อ + role badge + avatar + logout |
...
| Action | Access |
|--------|--------|
| Sign up | Admin only (via `supabase.auth.admin.createUser()`) |
...
> `profiles.role` = role ของ user ที่ login (ดึงจาก profiles table)

**profiles**
| Operation | Policy | คำอธิบาย |
|-----------|--------|---------|
| SELECT | `role = 'admin'` → ทุก row | Admin เห็นทุกคน |
| SELECT | `role = 'manager'` → เฉพาะ department เดียวกัน | Manager เห็นแค่ทีม |
| SELECT | `auth.uid() = id` | Employee เห็นแค่ตัวเอง |
| INSERT | `role = 'admin'` | Admin สร้าง user |
| UPDATE | `role = 'admin'` | Admin แก้ไข user |
| DELETE | `role = 'admin'` | Admin ลบ (soft delete) |

**missions**
| Operation | Policy |
|-----------|--------|
| SELECT | `role = 'admin'` → ทุก row |
| SELECT | `role = 'employee'` → เฉพาะที่ถูก assign (via user_missions join) |
| INSERT | `role = 'admin'` |
| UPDATE | `role = 'admin'` |
| DELETE | `role = 'admin'` |
...
|-----------|--------|
| SELECT | `role = 'admin'` → ทุก row |
| SELECT | `role = 'manager'` → เฉพาะ user ใน department เดียวกัน |
| SELECT | `auth.uid() = user_id` → Employee เห็นของตัวเอง |
| INSERT | `role = 'admin'` (assign) |
| UPDATE | `auth.uid() = user_id` AND status transition valid (employee: start/submit) |
| UPDATE | `role = 'manager'` AND user ใน department เดียวกัน (review/feedback) |
...
|-----------|--------|
| SELECT | `role = 'admin'` → ทุก row |
| SELECT | `role = 'manager'` → เฉพาะ department เดียวกัน |
...
| Operation | Policy |
|-----------|--------|
| SELECT | `role = 'admin'` OR `role = 'employee'` |
| INSERT | `role = 'admin'` |
| UPDATE | `role = 'admin'` |
| DELETE | `role = 'admin'` |
...
|-----------|--------|
| SELECT | `role = 'admin'` → ทุก row |
| SELECT | `role IN ('manager','employee')` → เฉพาะ is_active = true |
| INSERT | `role = 'admin'` |
| UPDATE | `role = 'admin'` |
| DELETE | `role = 'admin'` |
...
### 10.4 Supabase Edge Functions (ถ้าจำเป็น)
